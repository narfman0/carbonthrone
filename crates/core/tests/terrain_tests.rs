...
fn map_default_tile_is_open() {
    let map = generate_map(10, 10, ZoneKind::CommandDeck, 1, &[], &[], &mut rng());
...
fn map_dimensions_are_recorded() {
    let map = generate_map(12, 8, ZoneKind::DockingBay, 1, &[], &[], &mut rng());
...
fn zone_kind_is_recorded_on_map() {
    let map = generate_map(10, 10, ZoneKind::ResearchWing, 1, &[], &[], &mut rng());
    assert_eq!(map.zone_kind, ZoneKind::ResearchWing);
}

#[test]
fn map_contains_some_obstacles_for_excavation_site() {
    let map = generate_map(20, 20, ZoneKind::ExcavationSite, 1, &[], &[], &mut rng());
...
fn different_zones_produce_different_obstacle_densities() {
    ...
    let excavation = generate_map(20, 20, ZoneKind::ExcavationSite, 1, &[], &[], &mut rng1);
    let research = generate_map(20, 20, ZoneKind::ResearchWing, 1, &[], &[], &mut rng2);
...
fn isolated_tile_has_no_cover() {
    ...
    let map = generate_map(20, 20, ZoneKind::DockingBay, 1, &[], &[], &mut rng());
    let dirs = [
        Direction::North,
        Direction::South,
        Direction::East,
    ...
    for y in y0..rows as i32 {
        for x in 0..x1 {
            assert_eq!(
                map.get(x, y),
                Tile::Obstacle,
                "collapsed region tile ({},{}) should be Obstacle in loop 1",
                x,
                y
            );
        }
...
fn excavation_site_collapsed_section_opens_in_loop_3() {
    ...
    assert!(
        found_open,
        "collapsed region should have open tiles in loop 3"
    );
}
